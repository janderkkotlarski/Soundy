...
int sounding(std::vector <sf::Sound>& sounds, std::vector <sf::SoundBuffer>& buffers, const int which_sound)
{
	int count{0};
	assert(count == 0);
...
int window_loop(const std::string& program_name, std::vector <sf::Sound>& sounds, std::vector <sf::SoundBuffer>& buffers)
{
	...
	while (window.isOpen())
	{
		...
		which_sound = sounding(sounds, buffers, which_sound);
...
int main()
{
	...
	return window_loop(program_name, sounds, buffers);	
}
